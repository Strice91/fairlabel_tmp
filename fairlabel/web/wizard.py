...
class SetupWizard:
    ...
    def render_model_step(self):
        ...
        model_names = list(MODELS.keys())
        ui.radio(model_names, value=self.selected_model_name, on_change=self.on_model_select).props(
            "inline"
        ).classes("mb-4")

        if self.selected_model_name:
             ui.markdown(
                f"**{self.selected_model_name}** selected. "
                "Continue to configure hyperparameters."
